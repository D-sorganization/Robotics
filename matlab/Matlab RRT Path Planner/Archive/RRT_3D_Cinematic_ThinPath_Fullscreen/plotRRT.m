...
figure('Color','k','Units','normalized','OuterPosition',[0 0 1 1]); % Fullscreen figure
...
% Plot full path trace (thin)
plot3(path(:,1), path(:,2), path(:,3), 'c-', 'LineWidth', 0.5);
...
% Animate Falcon with small random roll
for i = 1:size(path_smooth,1)-1
    ...
    roll_angle = deg2rad(5*randn); % small random roll
    R_roll = [
        1 0 0;
    ...
    pause(0.0025); % fast cinematic
end

close(v);
title('3D Cinematic Falcon - Thin Path & Fullscreen','Color','w');
hold off;
end
