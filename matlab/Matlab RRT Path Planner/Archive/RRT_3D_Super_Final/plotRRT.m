...
if size(fv.vertices,2) == 2
    fv.vertices = [fv.vertices, zeros(size(fv.vertices,1),1)];  % pad to 3D if needed
end
original_vertices = fv.vertices;
falcon_plot = patch('Faces',fv.faces,'Vertices',original_vertices,'FaceColor',[0.8 0.8 0.8],'EdgeColor','none');

scale_factor = 0.1; % scaling factor
...
if ~isempty(path)
    for i = 1:size(path,1)-1
        ...
        cosA = cos(angle);
        sinA = sin(angle);
        Rz = [cosA -sinA 0; sinA cosA 0; 0 0 1];  % manual rotation about Z

        scaled_vertices = original_vertices * scale_factor;  % freshly scaled
        rotated_vertices = scaled_vertices * Rz';  % correct rotation
        translated_vertices = rotated_vertices + p1;  % correct translation
