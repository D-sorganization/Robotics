...
v = VideoWriter('falcon_flight_moving_final_fixed.mp4','MPEG-4');
open(v);
...
for frame = 1:size(path_smooth,1)-1
    ...
    for i = 1:size(obstacles,1)
        if isa(obstacle_patches(i),'Surface')
            % Sphere move
            ...
            for d = 1:3
                ...
                range = high - low;
                if d == 1
                    obstacle_patches(i).XData = mod(obstacle_patches(i).XData - low, range) + low;
                elseif d == 2
                    obstacle_patches(i).YData = mod(obstacle_patches(i).YData - low, range) + low;
                else
                    obstacle_patches(i).ZData = mod(obstacle_patches(i).ZData - low, range) + low;
                end
            end
        else
            % Cube move
