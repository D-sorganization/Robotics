...
%% Backtrack to Build the Final Path with Obstacle Avoidance  
path_nodes = [];  
if goal_reached  
    current = goal_node_id;  
    while current ~= -1  
        % Ensure path does not collide with obstacles  
        if current ~= 1  
            parent_id = tree.nodes(current, 4);  
            if check_collision(tree.nodes(parent_id, 2), tree.nodes(parent_id, 3), tree.nodes(current, 2), tree.nodes(current, 3), obstacles)  
                % Find an alternative parent that doesn't collide  
                valid_parent = find(~check_collision(tree.nodes(:,2), tree.nodes(:,3), tree.nodes(current, 2), tree.nodes(current, 3), obstacles), 1);  
                if ~isempty(valid_parent)  
                    tree.nodes(current, 4) = valid_parent;  
                else  
                    fprintf('No valid parent found, breaking path.\n');  
                    break;  
                end  
            end  
        end  
