function Tf = calculateTf(T, T_se_initial, T_standoff_initial, T_grasp, T_standoff_final, T_release)
    % Calculate T for each piece of the trajectory based on the Euclidean
    % distance between start and end point
    
    % If called with only T parameter, use the original evalin approach for backward compatibility
    if nargin == 1
