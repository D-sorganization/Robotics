...
class TrajectoryGenerator:
    ...
    def Generate_segment_trajectory(self, T_start, T_end, time_scale, traj_type):
        ...
        duration = max(
            dist / self.max_lin_vel, ang / self.max_ang_vel
        )  # Duration from rest to rest
        no_conf = int(
            (duration * self.k) / 0.01
        )  # Number of configuration between the start and goal

        if traj_type == "Screw":
            trajs = mr.ScrewTrajectory(T_start, T_end, duration, no_conf, time_scale)
        elif traj_type == "Cartesian":
            trajs = mr.CartesianTrajectory(
                T_start, T_end, duration, no_conf, time_scale
            )
    ...
    def convert_traj_csv_format(self, trajs, gripp_state):
        ...
        trajectory = list()
        for traj in trajs:
            new_traj = list(np.concatenate(traj[:-1, :-1], axis=0)) + list(
                np.concatenate(traj[:-1, -1:], axis=0)
...
if __name__ == "__main__":
    ...
    full_trajectory = traj_gen.convert_traj_csv_format(
        traj_gen.Generate_segment_trajectory(
            Tse_init, Tse_init_standoff, time_scale, traj_type
        ),
        gripper_open,
    )
    ...
    full_trajectory = full_trajectory + traj_gen.convert_traj_csv_format(
        traj_gen.Generate_segment_trajectory(
            Tse_init_standoff, Tse_init_grasp, time_scale, traj_type
        ),
        gripper_open,
    )
    ...
    # Close gripper
    full_trajectory = full_trajectory + traj_gen.convert_traj_csv_format(
        traj_gen.GripperChange(Tse_init_grasp), gripper_closed
    )
    ...
    full_trajectory = full_trajectory + traj_gen.convert_traj_csv_format(
        traj_gen.Generate_segment_trajectory(
            Tse_init_grasp, Tse_final_standoff, time_scale, traj_type
        ),
        gripper_closed,
    )
    ...
    full_trajectory = full_trajectory + traj_gen.convert_traj_csv_format(
        traj_gen.Generate_segment_trajectory(
            Tse_final_standoff, Tse_final_grasp, time_scale, traj_type
        ),
        gripper_closed,
    )
    ...
    # Open Gripper
    full_trajectory = full_trajectory + traj_gen.convert_traj_csv_format(
        traj_gen.GripperChange(Tse_final_grasp), gripper_open
    )
