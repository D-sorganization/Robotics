...
if __name__ == "__main__":
    ...
    # # New Task Settings
    Tsc_init = np.array([[1, 0, 0, 1.25], [0, 1, 0, 0], [0, 0, 1, 0.025], [0, 0, 0, 1]])

    Tsc_final = np.array(
        [[0, 1, 0, 0], [-1, 0, 0, -1.5], [0, 0, 1, 0.025], [0, 0, 0, 1]]
    )
    ...
    traj_list_complete, gripper_state_complete = tg.gen_full_trajectory(
        Tsc_init, Tsc_final, Tse_init, k, max_velocity, write_test_traj=False
    )
    ...
    for i in tqdm(range(len(traj_list_complete) - 1), position=0):
        ...
        u_theta, Xerr, Ki_error, J_e, V_t, Vd = fc.feedback_control(
            position_state, Xd, Xdnext, K, Ki, Ki_error, delta_t, test_joints
        )
        ...
        position_state = ns.next_state(
            position_state, velocity_state, delta_t, speed_limit=joint_speed_limit
        )
