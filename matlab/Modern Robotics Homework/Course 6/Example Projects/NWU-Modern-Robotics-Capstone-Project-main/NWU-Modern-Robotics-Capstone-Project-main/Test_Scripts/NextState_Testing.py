...
def calc_new_chassis_config(chassis_config, delta_theta_wheels):
    ...
    T_sb = np.array(
        [[1, 0, 0], [0, m.cos(phi), -1 * m.sin(phi)], [0, m.sin(phi), m.cos(phi)]]
    )
...
def next_state_test(
    position_state, gripper_state, velocity_state, iterations, max_time, speed_lim=12.5
):
    ...
    for i in range(iterations):
        position_state_new = next_state(
            position_state, velocity_state, speed_limit=speed_lim, delta_t=del_t
        )
...
gripper_state = 0
data_row = np.concatenate(
    (position_state[0], position_state[1], position_state[2], np.array(gripper_state)),
    axis=None,
).to_list
...
position_state_new = next_state(
    position_state, velocity_state, speed_limit=12.5, delta_t=0.5
)
