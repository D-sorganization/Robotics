...
class Robot:
    ...
    def __init__(self, bot_params, traj_params):
        ...
        self.traj_gen = TrajectoryGenerator(
            traj_params["max_lin_vel"], traj_params["max_ang_vel"], traj_params["k"]
        )

        # Feedback controller
        self.feedback_controller = FeedbackController(
            traj_params["Kp"], traj_params["Ki"], traj_params["timestep"], bot_params
        )
    ...
    def ComputeTrajectory(self):
        ...
        trajectories.append(
            [
                self.traj_gen.Generate_segment_trajectory(
                    self.Tse_init, Tse_init_standoff, self.time_scale, self.traj_type
                ),
                self.gripper_open,
            ]
        )
        ...
        trajectories.append(
            [
                self.traj_gen.Generate_segment_trajectory(
                    Tse_init_standoff, Tse_init_grasp, self.time_scale, self.traj_type
                ),
                self.gripper_open,
            ]
        )
        ...
        # Close gripper
        trajectories.append(
            [self.traj_gen.GripperChange(Tse_init_grasp), self.gripper_closed]
        )
        ...
        trajectories.append(
            [
                self.traj_gen.Generate_segment_trajectory(
                    Tse_init_grasp, Tse_init_standoff, self.time_scale, self.traj_type
                ),
                self.gripper_closed,
            ]
        )
        ...
        trajectories.append(
            [
                self.traj_gen.Generate_segment_trajectory(
                    Tse_final_standoff, Tse_final_grasp, self.time_scale, self.traj_type
                ),
                self.gripper_closed,
            ]
        )
        ...
        # Open Gripper
        trajectories.append(
            [self.traj_gen.GripperChange(Tse_final_grasp), self.gripper_open]
        )
...
if __name__ == "__main__":
    ...
    error_list = [
        [],
        [],
    ]  # Stores Xerr wrt to time.. [[Xerr1, Xerr2,..],[time1, time2,..]]

    kuka_bot = Robot(bot_params, traj_params)
    trajs = (
        kuka_bot.ComputeTrajectory()
    )  # Compute trajectory of end effector wrt space frame

    config = bot_params["initial_config"]
    # Adding initial config
    full_configs.append(
        [round(conf, 4) for conf in config["chasis"]]
        + [round(conf, 4) for conf in config["arm"]]
        + [round(conf, 4) for conf in config["wheel"]]
        + [config["gripp_state"]]
    )
    time = 0.0
    ...
    for traj, gripp_state in trajs:
        ...
        for conf_no in range(len(traj) - 1):
            ...
            end_eff_twist, wheel_vel, joint_vel = (
                kuka_bot.feedback_controller.FeedbackControl(
                    config, Tse_des, Tse_des_next
                )
            )
            error_list[0].append(time)
            error_list[1].append(end_eff_twist)
            controls = {"arm": list(joint_vel), "wheel": list(wheel_vel)}
            new_state = kuka_bot.odometry.NextState(
                config, controls, timestep
            )  # Compute next configuration after timestep
            full_configs.append(
                [round(conf, 4) for conf in new_state["chasis"]]
                + [round(conf, 4) for conf in new_state["arm"]]
                + [round(conf, 4) for conf in new_state["wheel"]]
                + [gripp_state]
            )
    ...
    for traj_seg in trajs:
        for traj_conf in traj_seg[0]:
            new_traj = list(np.concatenate(traj_conf[:-1, :-1], axis=0)) + list(
                np.concatenate(traj_conf[:-1, -1:], axis=0)
            )  # Converting into csv compatible
