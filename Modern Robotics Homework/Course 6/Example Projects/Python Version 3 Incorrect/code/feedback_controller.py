...
from math import copysign, cos, fabs, sin
...
class FeedbackController:
    ...
    def __init__(self, Kp, Ki, timestep, bot_params):
        ...
        self.Moe = np.array(
            bot_params["arm"]["Moe"]
        )  # Arm at home config (all joint angles zero).. end effector {e} wrt arm base {o}
        self.Blist = np.array(bot_params["arm"]["Blist"])  # Blist of arm

        row_val = 1.0 / (l + w)
        F = (self.rad / 4) * np.array(
            [[-row_val, row_val, row_val, -row_val], [1, 1, 1, 1], [-1, 1, -1, 1]]
        )
        self.F6 = np.array(
            [[0, 0, 0, 0], [0, 0, 0, 0], F[0], F[1], F[2], [0, 0, 0, 0]]
        )  # this maps change in wheel angle to body twist
    ...
    def FeedbackControl(self, config, Tse_des, Tse_des_next):
        ...
        )  # chasis base wrt space frame

        Toe = mr.FKinBody(
            self.Moe, self.Blist, thetalist
        )  # base of the arm wrt to end effector
        Tse_curr = np.dot(np.dot(Tsb, self.Tbo), Toe)  # Tse = Tsb * Tbo * Toe

        Tcurr_des = np.dot(
            mr.TransInv(Tse_curr), Tse_des
        )  # Transform of desired wrt to current
        Tdes_des_next = np.dot(
            mr.TransInv(Tse_des), Tse_des_next
        )  # Transform of next_desired wrt desired
        ...
        end_eff_twist = (
            feedforward_term + P_term + I_term
        )  # End effector twist wrt to space frame

        Teb = np.dot(
            mr.TransInv(Toe), mr.TransInv(self.Tbo)
        )  # chasis base wrt the end effector
        ...
        while True:
            vels = np.dot(
                np.linalg.pinv(J_e, 1e-3), end_eff_twist
            )  # [u,theta_dot].. tolerance of 1e-3 is added to pinv to avoid close to singularity situation
    ...
    def TestJointLimits(self, joint_vels, curr_joint_pos):
        ...
        for joint_no in range(len(joint_vels)):
            ...
            if fabs(joint_vels[joint_no]) > fabs(self.limit_joint_vel[joint_no]):
                print(
                    "Joint velocity exceeded for Joint ",
                    joint_no + 1,
                    joint_vels[joint_no],
                )
                joint_vels[joint_no] = copysign(
                    self.limit_joint_vel[joint_no], joint_vels[joint_no]
                )

            joint_pos[joint_no] = curr_joint_pos[joint_no] + (
                joint_vels[joint_no] * self.timestep
            )

            if (
                joint_pos[joint_no] > self.limit_joint_pos[joint_no][1]
                or joint_pos[joint_no] < self.limit_joint_pos[joint_no][0]
            ):
                constraint_joints.append(joint_no)
                print(
                    "Joint limit pos exceeded for Joint : ",
                    joint_no + 1,
                    joint_pos[joint_no],
                )
...
if __name__ == "__main__":
    ...
    feedback_controller = FeedbackController(
        params["Kp"], params["Ki"], params["timestep"], bot_params
    )

    end_eff_twist, wheel_vel, joint_vel = feedback_controller.FeedbackControl(
        params["config"], params["Tse_des"], params["Tse_des_next"]
    )
