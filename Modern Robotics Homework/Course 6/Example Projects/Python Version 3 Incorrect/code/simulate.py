...
import csv
from math import atan2, copysign, cos, fabs, sin
    ...
    def NextState(self, config, controls, timestep):
        ...
        for wheel_no in range(len(new_config["wheel"])):
            ...
            if fabs(controls["wheel"][wheel_no]) > fabs(self.limit_wheel[wheel_no]):
                print(
                    "Wheel velocity exceeded for wheel ",
                    wheel_no + 1,
                    controls["wheel"][wheel_no],
                )
                controls["wheel"][wheel_no] = copysign(
                    self.limit_wheel[wheel_no], controls["wheel"][wheel_no]
                )

            del_theta_wheel[wheel_no] = (
                controls["wheel"][wheel_no] * timestep
            )  # delta theta to be multiplied with F6 to find Vb6
            new_config["wheel"][wheel_no] = (
                config["wheel"][wheel_no] + del_theta_wheel[wheel_no]
            )
        ...
        for joint_no in range(len(new_config["arm"])):
            ...
            if fabs(controls["arm"][joint_no]) > fabs(self.limit_arm[joint_no]):
                print(
                    ...
                    joint_no + 1,
                    controls["arm"][joint_no],
                )
                controls["arm"][joint_no] = copysign(
                    self.limit_arm[joint_no], controls["arm"][joint_no]
                )

            del_theta_joint[joint_no] = controls["arm"][joint_no] * timestep
            new_config["arm"][joint_no] = (
                config["arm"][joint_no] + del_theta_joint[joint_no]
            )
        ...
        Vb6 = self.F6.dot(
            del_theta_wheel
        )  # Multiplied with timestep as Vb6 is the twist for unit time
...
if __name__ == "__main__":
    ...
    for iter in range(iterations):
        new_config = test_odom.NextState(
            params["config"], params["controls"], params["timestep"]
        )
        params["config"] = new_config
        configs.append(
            new_config["chasis"] + new_config["arm"] + new_config["wheel"] + [0]
        )  # chasis, arm, wheel, gripper state
