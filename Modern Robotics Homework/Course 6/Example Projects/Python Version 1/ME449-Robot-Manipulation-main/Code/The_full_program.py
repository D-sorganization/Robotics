...
def NextState(current_config, speed, timestep, max_omg):
    ...
    # Wheel speed
    u = np.array(
        [[speed_update[5]], [speed_update[6]], [speed_update[7]], [speed_update[8]]]
    )
    det_u = u * timestep
    ...
    next_config = np.concatenate(
        (next_chasis_config, next_joint_angles, next_wheel_angles), axis=None
    )
...
def TrajectoryGenerator(
    ...
    traj1 = np.asarray(
        mr.CartesianTrajectory(Tse_initial, Tse_init_standoff, Tf, N, method)
    )
    ...
    N = 100
    traj2 = np.asarray(
        mr.CartesianTrajectory(Tse_init_standoff, Tse_init_grasp, Tf, N, method)
    )
    ...
    gripper_state = 1
    traj3 = np.asarray(
        mr.CartesianTrajectory(Tse_init_grasp, Tse_init_grasp, Tf, N, method)
    )
    ...
    N = 100
    traj4 = np.asarray(
        mr.CartesianTrajectory(Tse_init_grasp, Tse_init_standoff, Tf, N, method)
    )
    ...
    N = 600
    traj5 = np.asarray(
        mr.CartesianTrajectory(Tse_init_standoff, Tse_fin_standoff, Tf, N, method)
    )
    ...
    N = 100
    traj6 = np.asarray(
        mr.CartesianTrajectory(Tse_fin_standoff, Tse_fin_grasp, Tf, N, method)
    )
    ...
    gripper_state = 0
    traj7 = np.asarray(
        mr.CartesianTrajectory(Tse_fin_grasp, Tse_fin_grasp, Tf, N, method)
    )
    ...
    N = 100
    traj8 = np.asarray(
        mr.CartesianTrajectory(Tse_fin_grasp, Tse_fin_standoff, Tf, N, method)
    )
...
def main(Tsc_ini, Tsc_fin, Kp, Ki, robot_config):
    ...
    traj = np.asarray(
        TrajectoryGenerator(Tse_ini, Tsc_ini, Tsc_fin, Tce_grp, Tce_sta, k)
    )
    # print(traj)
    ...
    for i in range(1699):  # 14s
        ...
        Tsb = np.array(
            [
                [cos(robot_config[0]), -sin(robot_config[0]), 0, robot_config[1]],
        ...
        command_V, X_err = FeedbackControl(
            X, Xd, Xd_next, Kp, Ki, timestep, integral, robot_config
        )
...
kp = 20
ki = 0.1
Tsc_ini = np.array([[0, -1, 0, 1], [1, 0, 0, 1.0], [0, 0, 1, 0.025], [0, 0, 0, 1]])
Tsc_fin = np.array([[1, 0, 0, 2], [0, 1, 0, 0], [0, 0, 1, 0.025], [0, 0, 0, 1]])
