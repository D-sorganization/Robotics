...
%% Robot parameters (matching working project exactly)
% Mobile base parameters - use exact values from working project
l = 0.47/2;   % half-length of robot (m) - matching working project
w = 0.3/2;    % half-width of robot (m) - matching working project
...
switch task_type
    case 'best'
        % Well-tuned controller matching working project exactly
        Kp = 1.5 * eye(6);
        Ki = 0 * eye(6);  % Working project uses zero integral gain
        
    case 'overshoot'
        % Higher gains to demonstrate overshoot behavior
        Kp = 20 * eye(6);
        Ki = 0.5 * eye(6);
        
    case 'newTask'
        % Optimized gains for new task based on analysis
        Kp = 1.8 * eye(6);  % Slightly higher for faster response
        Ki = 0.05 * eye(6); % Small integral to reduce steady-state error
        
    case 'optimal'
        % New optimized controller based on working project analysis
        Kp = diag([2.0, 2.0, 2.0, 1.5, 1.5, 1.5]); % Higher gains for rotation, moderate for translation
        Ki = diag([0.02, 0.02, 0.02, 0.01, 0.01, 0.01]); % Small integral gains
        
    case 'robust'
        % Conservative gains for maximum stability
        Kp = 1.2 * eye(6);
        Ki = 0 * eye(6);
        
    otherwise
        % Default to best controller for unknown types
        fprintf('Warning: Unknown controller type "%s", using "best" controller\n', task_type);
        Kp = 1.5 * eye(6);
        Ki = 0 * eye(6);
end

% Debug: Display selected gains
fprintf('Controller gains selected for %s:\n', task_type);
fprintf('  Kp diagonal: [%.2f, %.2f, %.2f, %.2f, %.2f, %.2f]\n', diag(Kp));
fprintf('  Ki diagonal: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f]\n', diag(Ki));
...
% Joint limits to avoid singularities and self-collision (like working project)
joint_limits = [pi, -pi;      % Joint 1 [max; min]
                pi, -pi;      % Joint 2  
                pi, -pi;      % Joint 3
                pi, -pi;      % Joint 4
                pi, -pi];     % Joint 5
...
for i = 1:N_traj-1
    ...
    % Use error history for integral control (like working project)
    if i == 1
        Xerr_history = [];
    else
        Xerr_history = Xerr_log(:, 1:i-1);
    end
    
    % Feedback control with corrected interface
    [V, Xerr] = FeedbackControl(X, Xd, Xd_next, Kp, Ki, dt, Xerr_history);
    
    % Store current error
    Xerr_log(:,i) = Xerr;
    
    % Check and handle joint limits (simplified version)
    ...
    % Apply joint limits by zeroing corresponding Jacobian columns
    if ~isempty(violated_joints)
        for j = violated_joints
            Je(:, j+4) = zeros(6, 1);  % Zero the arm joint columns
        end
    end
    
    % Calculate joint speeds using pseudoinverse (like working project)
    speeds = pinv(Je, 0.009) * V;  % Use same tolerance as working project
