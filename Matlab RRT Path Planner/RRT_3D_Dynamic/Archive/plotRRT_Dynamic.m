...
% plotRRT_Moving.m
% Updated for wide camera view and full scene framing (April 2025)

figure('Color','k','Units','normalized','OuterPosition',[0 0 1 1]);
hold on; grid on; axis equal vis3d;
...
campos([0, -5, 2]);   % Farther camera back
camtarget([0, 0, 0]); % Look at scene center
camup([0, 0, 1]);
view(3);              % 3D perspective
camva(30);            % Wider field of view

% Load Falcon STL
fv = stlread('falcon_clean_fixed.stl');
original_vertices = fv.Points;
falcon_plot = patch('Faces',fv.ConnectivityList,'Vertices',original_vertices, ...
