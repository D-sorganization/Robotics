...
if ~isempty(path)
    for i = 1:size(path,1)-1
        ...
        % Camera follow (tighter zoomed-in view)
        behind_offset = [-0.1 -0.1 0.05]; % much closer behind
        campos(p1 + behind_offset);
        camtarget(p1);
        camup([0 0 1]); % maintain z-up
        ...
        if i < size(path,1)-20
            pause(0.1); % cinematic slower pace
        else
            pause(0.02); % hyperspeed at end
        end
    end
end

close(v);
title('3D Cinematic Falcon Chase - Zoomed In','Color','w');
hold off;
end
