...
if ~isempty(path)
    for i = 1:size(path,1)-1
        ...
        % Camera follow (closer, cinematic angle)
        behind_offset = [-1.0 -1.0 0.5];  % closer chase cam
        campos(p1 + behind_offset);
        camtarget(p1);
        camup([0 0 1]); % keep Z up
        ...
        if i < size(path,1)-20
            pause(0.1); % slower cinematic movement
        else
            pause(0.02); % hyperspeed jump
        end
    end
end

close(v);
title('3D Cinematic Falcon Space Chase','Color','w');
hold off;
end
