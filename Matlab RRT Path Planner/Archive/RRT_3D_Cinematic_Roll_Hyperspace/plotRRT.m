...
stars = plot3(star_x, star_y, star_z, 'w.', 'MarkerSize', 2);
...
% Animate Falcon with roll + star stretch
for i = 1:size(path_smooth,1)-1
    ...
    % Small random roll
    roll_angle = deg2rad(5*randn); % small roll variation
    R_roll = [
        1 0 0;
    ...
    % Hyperspace star stretch at end
    if i > 400
        % Stretch stars outward
        star_scale = 1 + (i-400)*0.05; % scale factor
        set(stars, 'MarkerSize', 2 + (i-400)*0.5);
    end
...
close(v);
title('3D Cinematic Falcon with Roll + Hyperspace','Color','w');
hold off;
end
